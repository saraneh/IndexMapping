...
### Step Two:
**Once your image has been georeferenced, use map to digitize the index map polygons.** Each index map will be one polygon, and every polygon will be stored in one single shapefile. Some polygons will not have an index map linked to it, but all index maps link to exactly one polygon.
### Step Three:
